...
// ini_set('display_errors', 1);
	   ...
	   {
	       $bookings =  $this->db->get_where('booking', array('car_id'=>$cdata->car_id,'payment_status'=>1))->result();
	...
	{   
	   ...
	   if(isset($_GET["start"]) && isset($_GET["end"]) && isset($_GET["vehicle_number"])){
            ...
            $car = $this->db->select('*')->get_where('car', array('vehicle_number'=>$vehicle_number))->row();
                 ...
                 $s = date('Y-m-d H:i:s',strtotime($_GET["start"]));
                 $e = date('Y-m-d H:i:s',strtotime($_GET["end"]));
                 $diff_begin = new DateTime($s);
                $diff_end = new DateTime($e);
                $interval = DateInterval::createFromDateString('1 hour');
           ...
           //  $bookings =  $this->db->get_where('booking', array('car_id'=>$car->car_id,'start >= '=>$s))->result();
              $bookings =  $this->db->get_where('booking', array('car_id'=>$car->car_id,'payment_status'=>1))->result();
             $booking = check_booking($bookings,$s,$e);
             if($booking == 0){
                
                 $data = $this->db->query( "SELECT * FROM car_hide_history  
    	         ...
    	         if(($car->sold_from!="0000-00-00 00:00:00")&&($car->sold_to!="0000-00-00 00:00:00")) {
                      foreach ($period_diff as $dt) {
                          $dateDiff = $dt->format("Y-m-d H:i:s");
                          if( ($dateDiff >= $car->sold_from) && ($dateDiff <= $car->sold_to) ){
                              $booking = 1;
                          }
                      }
                    }
