...
class Data_model extends CI_Model
{
	...
	public function updateSystemSetting()
	{
        ...
        $data['description'] = $this->input->post("price_decrease_percentage");
        $this->db->where('type','price_decrease_percentage');
        $this->db->update('settings',$data);
        
         $data['description'] = $this->input->post("down_start_time");
        $this->db->where('type','down_start_time');
        $this->db->update('settings',$data);

        $data['description'] = $this->input->post("down_end_time");
        $this->db->where('type','down_end_time');
        $this->db->update('settings',$data);
    ...
    public function testimonial_image()
    {
      ...
      public function testimonial_add($name,$review)
    {
          $data['name'] = $name;
           $data['review'] =$review;
        ...
        {
            $data['image']    = $_FILES['b_image']['name'];
            $file_size    = $_FILES['b_image']['size'];
            $file_tmp     = $_FILES['b_image']['tmp_name'];
            $file_type    = $_FILES['b_image']['type'];
            $path         =  "uploads/testimonial_bg/";
          
            $temp = explode(".", $_FILES["b_image"]["name"]);
...
public function delete_slider_image($param2, $image_path)
    ...
    public function insert_blog()
    {
        ...
        $data['slug'] = $this->slugify($this->input->post('title'));
    ...
    {
        // print_r(count($this->input->post('hide_to')));
        // die;
        ...
        // $data['hide_from']      =   $this->input->post('hide_from')[0];
        // $data['hide_to']        =   $this->input->post('hide_to')[0];
        ...
        $data['car_lat']  =   $this->input->post('lat');
        $data['car_long']  =   $this->input->post('long');
        ...
        $data = $this->db->insert('car', $data);
        $carId = $this->db->insert_id();
        
        // print($carId);die;
        
        
        // ============================= CAR HIDE FUNCTIONALITY =========================================
            // $hideFrom = $this->input->post('hide_from');
            // $hideTo = $this->input->post('hide_to');
            // $hideReason = $this->input->post('hide_reason');
            // $data = [];
            // foreach($hideFrom as $key => $value)
            // {
            //     $arr = [
            //         'hide_from' => $hideFrom[$key],
            //         'hide_to' => $hideTo[$key],
            //         'car_id' => $carId,
            ...
            //     array_push($data,$arr);
            // }
            // $this->db->insert_batch('car_hide_history', $data); 
        // ============================= CAR HIDE FUNCTIONALITY =========================================
    ...
    public function update_car($param2)
    {
       ...
       // $data['sold_from']  =   $this->input->post('sold_from');
       // $data['sold_to']  =   $this->input->post('sold_to');
       // $data['sold_remark']  =   $this->input->post('sold_remark')[0];
        ...
        // ============================= CAR HIDE FUNCTIONALITY =========================================
            $this->db->where('car_id',$param2)->delete('car_hide_history'); 
            
            $hideFrom = $this->input->post('sold_from');
            $hideTo = $this->input->post('sold_to');
            $hideReason = $this->input->post('sold_remark');
            ...
            foreach($hideFrom as $key => $value)
            {
                $arr = [
                    ...
                    'hide_to' => $hideTo[$key],
                    'car_id' => $carId,
                    'hide_reason' => $hideReason[$key]
                ];
                array_push($data2,$arr);
            }
            
            $this->db->insert_batch('car_hide_history', $data2); 
        // ============================= CAR HIDE FUNCTIONALITY =========================================
    }
    ...
    {
        $bookingId = base64_decode($_POST['bookingId']);
        // print($bookingId);die;
        
        $bookingData = $this->db->where([
        ...
        $up = $this->db->where(['booking_id' => $bookingId])->update('booking',[
            'payment_status' => 0,
            'cancel_remark_admin' => $remark,
            'cancel_percentage' => $per
        ]);
        
        $user_id = $bookingData->user_id;
        
        $this->db->insert('booking_cancell', [
            'booking_id' => $bookingId,
            'user_id' => $user_id,
            'order_status' => 'accept',
