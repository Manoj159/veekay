		...
		<div class="container" data-aos="fade-up">
			<div class="row mt-3">
				<div class="col-md-6 offset-3 mt-5 mt-lg-0">
					<div class="card">
						<form action="<?= base_url(); ?>Checkout/do_payment" id="payment" method="post">
							<input type='hidden' class="only_security_amount" name='only_security_amount' value='0'>
							...
							<div class="card-body" id="fares">
								...
								<?php 
									...
									?>
								<?php  
									$starttimestamp = strtotime($this->session->userdata('start'));
									$endtimestamp = strtotime($this->session->userdata('end'));
									$difference = abs($endtimestamp - $starttimestamp)/3600;
									if($difference < 24 && $this->session->userdata('city')==6){
									                 $final_car_price =  ($car->price*24);
									                  $total_payable = $final_car_price+$gst+$refund;
									                 $this->session->set_userdata('total_payable',$total_payable);
									
									 
									}else{
									 if($this->session->userdata('city')== 6){
									             ...
									             if( $endTime > 9){
									                  $timeMinus= ($endTime-9);
									                $total_payable = ($total_payable+($car->price*24)-($car->price*$timeMinus));
									                 $final_car_price =  ($final_car_price+($car->price*24)-($car->price*$timeMinus));
									                 $this->session->set_userdata('total_payable',$total_payable);
									...
									?>
								<p>Number Of Days / Hours <label class="float-right"><?= @$main_date; ?></label></p>
								<?php
									if($this->session->userdata("coupon_discount")){
									?>
								...
								<p>
									Refundable Deposit <label class="float-right"><i class='bx bx-rupee'></i><?= $refund; ?></label><br/>
							...
							<!-- <div class="card-footer" id="submit_fares">
								<button type="submit" class="btn book_distributes">PROCEED TO PAY(<i class='bx bx-rupee'></i><?php echo $total_payable ?>)</button>
								</div>
								
								
								<div class="card-footer" id="submit_fares">
								<button type="button" title="<?php echo 'Balance Amount is payable '. $total_payable - $refund . ' Rs. on delivery of car.'?>" class="btn book_distributes only_security">PROCEED TO PAY Only Security Amount (<i class='bx bx-rupee'></i><?php echo $refund ?>)</button>
								</div> -->
							<button type="submit" class="btn book_distributes w-50">PAY IN FULL</button>
							<button type="button" title_text="Balance amount of <?php echo $total_payable - $refund; ?>/- is payable upon car delivery."  data-bs-toggle="tooltip" data-bs-placement="top" class="btn book_distributes only_security w-50" style="float: right" >PAY ONLY DEPOSIT</button>
						</form>
					</div>
				</div>
			</div>
...
<script>
	$('.only_security').on('click', function(){
	    swal({
	      title: "",
	      text: $(this).attr('title_text'),
	      icon: "info",
	      buttons: true,
	    })
	    .then((value) => {
	        ...
	        if( value == null)
	        {
	            ...
	            $('.only_security_amount').attr("value",1);
	            $('#payment').submit();
	        }
	    });
